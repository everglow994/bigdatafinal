...
def create(I, U, K, hidden_activation, output_activation, q=0.5, l=0.01):
    ...
    # drop out corruption: the corrupted level is determined by q
    h_item = Dropout(q)(x_item)

    # Fully connected layer
    h_item = Dense(K, kernel_initializer='random_uniform',
                bias_initializer='random_uniform', W_regularizer=l2(l), b_regularizer=l2(l))(h_item)

    # dtype should be int to connect to Embedding layer
    x_user = Input((1,), dtype='int32', name='x_user')

    # embedding layer into K dimension
    h_user = Embedding(input_dim=U, output_dim=K, input_length=1, W_regularizer=l2(l))(x_user)
    # flatten layer to generate user latent vector
    h_user = Flatten()(h_user)

    # merge user input node with item input matrix
    ...
    # output mapping and output activation
    y = Dense(I, kernel_initializer='random_uniform',
                bias_initializer='random_uniform', W_regularizer=l2(l), b_regularizer=l2(l), activation=output_activation)(h)
